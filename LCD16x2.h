 ...
 * Modified by @jhairssteven on 5 july 2022
 */
...
#ifndef _XTAL_FREQ
#error Macro _XTAL_FREQ must be set to MCU clock base frecuency for LCD16x2 lib to work properly
#endif
...
void ConfiguraLCD(unsigned char a) {
    if (a == 4 | a == 8) interfaz = a;
}

/*Cuando en modo 4 bits, se utiliza el nibble inferior o derecho del puerto de datos*/
void EnviaDato(unsigned char a) {
    if (interfaz == 4) {
        Datos = ((Datos & 0b00001111) | (a & 0b11110000)) >> 4;
        HabilitaLCD();
        RetardoLCD(1);
        Datos = ((Datos & 0b00001111) | (a << 4)) >> 4;
        // HabilitaLCD();
        // RetardoLCD(1);
    } else if (interfaz == 8) {
        Datos = a;
...
void EscribeLCD_n8(unsigned char a, unsigned char b) {
    ...
    // b es el n�mero de digitos que se desea mostrar empezando desde las
    // unidades Ejemplo EscribeLCD_n8(204,3);
...
void EscribeLCD_n16(unsigned int a, unsigned char b) {
    ...
    // b es el n�mero de digitos que se desea mostrar empezando desde las
    // unidades Ejemplo EscribeLCD_n16(12754,5);
...
void FijaCursorLCD(unsigned char fila, unsigned char columna) {
    ...
    // columna es un valor positivo que especifica la posici�n del cursor de 1 a
    // 80 Si el display es de una fila, Ej: 8x1, tiene 80 columnas Si el display
    // es de dos filas, Ej: 20x2, tiene 40 columnas Si el display es de cuatro
    // filas, Ej: 20x4, tiene 20 columnas
}
void DesplazaPantallaD(void) {
